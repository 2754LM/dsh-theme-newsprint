...
全部颜色收敛成一份调色板（`lib/client.js` 的 `PALETTE`），改一个值全局跟着走。
...
墨阶（对 `base` 的对比度）：正文 `#1f0909` 17.2:1、次级 `#5a5049` 7.1:1、主文本弱化 `#6b6156` 5.5:1、三级 `#776c62` 4.6:1、题注 `#948a7e` 3.0:1（只用于元信息）、装饰 `#c3bbae`。

唯一强调色是海蓝 `#065588`（7.1:1），只管链接、焦点与信息态；品牌色和主按钮用墨色，悬停加深为 `#3f2d22`；绿 / 琥珀 / 红一律降饱和到印刷油墨档。

两条硬约束：**中性色必须落在上面的暖纸阶里——纯中性灰（r=g=b）放在暖纸上会显脏**；**半透明色一律由 `alpha()` 从不透明源色派生，不手写 rgba**。
